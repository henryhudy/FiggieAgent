...
% ----------------------------------------------------------------------
\title{\textbf{Bayesian Card-Consistency Features and Deep CFR\\
in a Reduced Figgie Trading Game}}
...
\begin{document}
\input{deep_cfr_focus}
\end{document}

% Legacy manuscript retained below for reference. It is deliberately not
% compiled: the current paper reports only the reduced-game Bayesian/Deep-CFR
% study, while the older MCTS study remains available in repository history.
\maketitle

% ----------------------------------------------------------------------
\begin{abstract}
\noindent
We study a concrete question about decision making under partial
observability: when does a more accurate latent-state belief improve control?
Our test bed is \emph{Figgie}, a small general-sum trading game with a
12-way hidden suit-count assignment and hidden opponent hands. We build a
discretized simulator, maintain an \emph{exact card-consistency posterior}
over the hidden assignment, and train an information-set Monte Carlo tree
search agent with neural policy, value, and history-conditioned belief heads.
The posterior is calibrated against simulator truth (ECE $0.02$--$0.08$).
We then evaluate both intermediate inference and downstream decisions under
two controlled opponent pools. On a random pool, stronger belief machinery
does not improve the main control metric after the first neural-prior step.
On a mixed pool whose opponents' actions encode more information, the learned
belief and its product combination with the exact posterior improve the
reported single-seed A/B result by $+5.25$ and a further $+5.75$ mean dollars
per round. This contrast is the paper's central result: calibrated latent
state estimation matters for control only when the environment exposes
decision-relevant behavioral signal. We also report a negative result:
bundle quotes are engine-correct but are posted on 63\% of turns and fill in
only 1.4\% of trades against our pools. Finally, a \emph{tabular},
chance-sampled CFR baseline in a reduced two-player game is evaluated with an
exact reduced-game exploitability calculation; it is not Deep CFR and does
not establish equilibrium quality for full Figgie. All conclusions are
simulator- and fixed-pool-specific, with frozen checkpoints and
seed-configurable evaluation code released for replication.
\end{abstract}
...
This paper is a controlled study of the link between state estimation and
action selection in a small imperfect-information game. It does not claim to
measure broad reasoning ability. We make the following contributions.
...
\begin{enumerate}[leftmargin=1.5em]
  ...
  chance).
  \item \textbf{A belief-conditioned control agent.} We combine
  belief-consistent determinized search, neural policy/value priors, a
  history-conditioned belief head, and population self-play. Against three
  random bots it wins $95$--$97\%$ of rounds with mean round delta
  $+\$126\pm5$ in the released evaluation setup.
  \item \textbf{A controlled inference-to-control finding.} The random-pool
  metric plateaus after the first neural-prior step, while the learned belief
  helps in the behavior-driven mixed pool. This establishes a conditional
  result within the simulator, not a claim of general reasoning or broad
  ...
  \item \textbf{A reduced-game diagnostic.} We train a tabular,
  chance-sampled CFR policy in a two-player zero-sum reduction and compute
  exploitability exactly for that reduction. The policy transfers as a
  search-free baseline to the full pool, but neither the algorithm nor the
  guarantee extends to full Figgie (\S\ref{sec:cfr}).
...
\citep{he2016opponent} and adopt OpenSpiel's conventions for
...
This protocol separates two questions that are often conflated: whether a
model estimates the hidden assignment accurately, and whether that estimate
improves action selection. The random and mixed pools are interventions on
the availability of behavioral signal, not external benchmarks. The
single-seed mixed-pool A/B values are therefore evidence for a hypothesis to
be replicated across independent seeds, not a population-level estimate or a
claim about general reasoning.
...
domination. To add a game-theoretic diagnostic, we reduce Figgie to a
two-player, zero-sum subgame, \emph{Figgie-Lite}, and train a \emph{tabular}
policy with chance-sampled CFR \citep{zinkevich2008regret, lanctot2009mccfr}.
This is not Deep CFR, and it does not solve the full game. Figgie-Lite keeps the
...
both players' actions exactly at every node; chance is sampled during CFR
training, while the reported strategy value and best-response exploitability
are evaluated exactly in the reduction. The resulting average strategy is a
strict improvement over the hand-crafted references and has low measured
exploitability in Figgie-Lite.
...
We presented a reproducible study of belief-conditioned control in Figgie:
exact card-consistency inference, information-set search, neural priors, and
population self-play, backed by a property-verified engine and component-level
calibration checks (\S\ref{sec:vv}). The central result is conditional: after
the first neural-prior improvement, stronger belief machinery does not improve
the random-pool control metric, but it improves the reported mixed-pool A/B
comparison when opponents' behavior carries signal. The resulting hypothesis
is not that the agent has general reasoning ability. It is that an improved
latent-state representation affects control only when the observation process
contains information the policy can exploit. The bundle and value-head results
are useful negative controls: a correct mechanism is not automatically useful,
and calibrated state features do not by themselves guarantee better action
selection. Figgie-Lite supplies a separate tabular CFR diagnostic with exact
evaluation only in that reduced game. The released harness makes these claims
straightforward to rerun and extend with multi-seed evaluations.
